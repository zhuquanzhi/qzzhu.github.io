    ...
    UE.commands['edittable'] = {
        ...
        execCommand: function (cmd, pros) {
            ...
            if (table) {
                ...
                if(pros.border)
	                table.setAttribute('border',pros.border);
	              if(pros.color)
	              	table.setAttribute('bordercolor',pros.color);
                if(pros.width)
  	              table.setAttribute('width',pros.width);
                if(pros.height)
    	            table.setAttribute('height',pros.height);
                if(pros.cellpadding)
      	          table.setAttribute('cellpadding',pros.cellpadding);
                if(pros.cellspacing)
                	table.setAttribute('cellspacing',pros.cellspacing);
                utils.each(arr, function (node) {
                		if(pros.color)
                    	node.style.borderColor = pros.color;
                });
            }
        }
    };
...
UE.commands['insertparagraphBefore'] = {
    execCommand : function( cmdName,front) {
        var me = this,
            range = me.selection.getRange(),
            start = range.startContainer,tmpNode;
        ...
        if(tmpNode){
            var p = me.document.createElement('p');
            if(front){
                tmpNode.parentNode.insertAfter(p,tmpNode)
            }else{
                tmpNode.parentNode.insertAfter(p,tmpNode.nextSibling)
            }
            domUtils.fillNode(me.document,p);
            range.setStart(p,0).setCursor(false,true);
        }
