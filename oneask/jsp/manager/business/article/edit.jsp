	...
	<div id="left_layout">
		...
		<div id="main_content" class="container-fluid">
			<ul class="breadcrumb breadcrumb-main">
				...
				<li class="text-info">编辑文章</li>
			</ul>
			<div class="fixed-top clearfix">
				<div class="left labtitle">
					<i class="icon-minus i-rotate-d"></i>编辑文章
				</div>
			</div>
			<form action="${ctx}/manager/business/article/update" method="post" id="myform">
			
			<input name="article.id" type="hidden" value="${article.id}">
			<input name="article.comment_count" type="hidden" value="${article.comment_count}">
			<input name="article.browse_count" type="hidden" value="${article.browse_count}">
			<div class="well widget">
				...
				<div class="excerpt">
					<div class="excerpt-span-long">
						...
						<input class="validate[required,maxSize[350]] span7" type="text" name="article.title" value="${article.title}"></span>
					</div>
					<p class="excerpt excerpt-wid clearfix">
						<span class="excerpt-span"> <label class="text-success">文章作者：</label> <span class="input-pro-det"><input class="validate[required,maxSize[50]] span3" type="text" name="article.author"  value="${article.author}"></span>
						</span> 
						<span class="excerpt-span"> <label class="text-success">文章头衔：</label> <span class="input-pro-det"><input class="validate[required,maxSize[50]] span3" type="text" name="article.honor" value="${article.honor }"></span></span>
						<span class="excerpt-span"> <label class="text-success">发布时间：</label> <span class="input-pro-det"><input class=" dateWithDay datetimePicker span2 validate[required]" type="text" name="article.push_time" value='<fmt:formatDate value="${article.push_time}"/>'></span>
						</span>
					...
					<div class="excerpt excerpt-wid clearfix">
						<span class="excerpt-span">
							...
							<label class="radio"><span><input type="radio" name="article.is_show" value="0" class="fancy" style="opacity: 0;" <c:if test="${article.is_show eq 0 }"> checked="checked"</c:if>></span>是</label> 
							<label class="radio"><span><input type="radio" name="article.is_show" value="1" class="fancy"  style="opacity: 0;"<c:if test="${article.is_show eq 1 }"> checked="checked"</c:if>></span>否</label>
							</span>
						</span> 
						<span class="excerpt-span"> 
							<label class="text-success">是否原创： </label> 
							<span class="input-pro-det"> 
							<label class="radio"><span><input type="radio" name="article.is_original" value="0" class="fancy" style="opacity: 0;" <c:if test="${article.is_original eq 0 }"> checked="checked"</c:if> ></span>是</label> 
							<label class="radio"><span><input type="radio" name="article.is_original" value="1" class="fancy"  style="opacity: 0;" <c:if test="${article.is_original eq 1 }"> checked="checked"</c:if>></span>否</label>
							</span>
						</span>
						<span class="excerpt-span"> 
							<label class="text-success">是否推荐： </label> 
							<span class="input-pro-det"> 
							<label class="radio"><span><input type="radio" name="article.is_recommend" value="0" class="fancy" style="opacity: 0;" <c:if test="${article.is_recommend eq 0 }"> checked="checked"</c:if> ></span>是</label> 
							<label class="radio"><span><input type="radio" name="article.is_recommend" value="1" class="fancy"  style="opacity: 0;" <c:if test="${article.is_recommend eq 1 }"> checked="checked"</c:if>></span>否</label>
							</span>
						</span>
					</div>
					<div class="excerpt-span-long">
						<label class="text-success">文章标签：</label> <span>
							<p>
								<c:set var="tagStr" value=";${article.tag};"></c:set>
								<c:forEach items="${tags}" var="item">
									<label class="tuozhiyx_span"> <input type="checkbox" value="${item.id}" name="tags" value="${item.id}" class="fancy" <c:if test="${fn:contains(tagStr,';'.concat(item.id).concat(';'))}"> checked="checked"</c:if>/>${item.name }</label> 
								</c:forEach>
							</p>
					...
					<div class="excerpt-span-long">
						<label class="text-success">文章简介：</label> <span> <textarea rows="4" name="article.summary" class="input-block-level">${article.summary }</textarea>
						</span>
					</div>

					<div class="excerpt-span-long">
						<label class="text-success">视频地址： </label> <span> <input class="span5" type="text" name="article.video" value="${article.video}">
						</span>
					</div>
					...
					<div class="panel-body">
						<div id="accorder_b" class="info_content_big">
							<div class="photo-list clearfix">
								...
								<c:if test="${not empty article.cover_image}">
									<li>
										<img src="${ctxfile}${article.cover_image}"><!-- data-toggle="lightbox"  href="#image-lightbox" -->
										<a rel="delete-picture"><i class="icon-remove color-red"></i></a>																			
										<input type="hidden" name="article.cover_image"  value="${article.cover_image}"/>
									</li>
								</c:if>
								</ul>
                                <ul class="clearfix">
	                            	<li class="pic-plus" id="img-upload" <c:if test="${not empty article.cover_image}">style="display: none;"</c:if>><a id="uploadPicture"><i class="icon-plus"></i>添加图片</a></li>
                                </ul>
                               <%--  <div id="image-lightbox" class="lightbox hide fade"  tabindex="-1" role="dialog" aria-hidden="true">
									<div class='lightbox-content'>
										<img src="${ctxfile}${article.cover_image}">
									</div>
								</div> --%>
                            </div>
						</div>
								...
								<div class="panel-body">
									<div id="accorder_b" class="info_content_big">
										<div class="excerpt clearfix">
											...
											<span class="excerpt-span-long"> <span class="input-pro-det attachment-box">
												<c:forEach items="${files}" var="item">
													<span>
														<label class="attachment" data-value="${item.id}" title="${item.name}">
															<c:choose>
																<c:when test="${fn:length(item.name)>25}">${fn:substring(item.name,0,24)}...</c:when>
																<c:otherwise>${item.name}</c:otherwise>
															</c:choose>
														</label>
														<i class="icon-download-alt"></i><i class="icon-remove"></i>|
													</span>
												</c:forEach>
											 </span>
											</span>
						...
						<div class="panel panel-default">
							...
							<div id="collapse-four" class="panel-collapse in collapse">
								<div class="panel-body">
									...
									<textarea style="width: 80%;" name="article.content" id="newsDesc">${article.content}</textarea>
								</div>
							</div>
						</div>
					</div>
...
<script type="text/javascript" charset="utf-8">
	...
	var _editor_picture = new baidu.editor.ui.Editor({
	   ...
	   fileParams:{"savePath":"article","saveFolder":"article/attachments","perfixName":"article","articleId":${article.id}},
